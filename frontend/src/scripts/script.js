...
for (let i = 0; i < 25; i++) {
  ...
  star.setAttribute(
    ...
    `
        ...
        --star-speed: ${120 / (0.1 + Math.random() * 0.5)}s;
...
main.addEventListener(
  ...
  function (event) {
    ...
    if (!scrollbarToggler) {
      toggleScrollbar(
        'inset 0 0 6px var(--secondary-color)',
        'solid var(--secondary-color) 2px',
        'auto'
      );
...
document.addEventListener('DOMContentLoaded', function () {
  if ('IntersectionObserver' in window) {
      ...
      function (entries, observer) {
        entries.forEach(function (entry) {
          if (
            ...
            !entry.target.classList.contains('lazy-loaded')
          ) {
    ...
    let activateObserver = new IntersectionObserver(
      function (entries, observer) {
        entries.forEach(function (entry) {
...
orderElements(
  document.querySelectorAll('.sphere2 .hemisphere'),
  '--nth-parent'
);
orderElements(
  document.querySelectorAll('.show-text:nth-child(n-1)'),
  '--nth-child'
);
orderElements(
  document.querySelectorAll('.sphere .hemisphere:nth-child(1) .skill'),
  '--nth-child'
);
orderElements(
  document.querySelectorAll('.sphere .hemisphere:nth-child(2) .skill'),
  '--nth-child'
);
orderElements(
  document.querySelectorAll('.sphere2 .hemisphere:nth-child(1) .skill'),
  '--nth-child'
);
orderElements(
  document.querySelectorAll('.sphere2 .hemisphere:nth-child(2) .skill'),
  '--nth-child'
);
